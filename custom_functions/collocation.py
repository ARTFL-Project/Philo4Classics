        ...
        for hit in hits[hits_done:]:
            ...
            if parent != stored_sentence_id:
                ...
                if collocate_distance:
                    ...
                    row_query = """select philo_name,lemma from words where parent='%s' and rowid between %d and %d""" % (
                        parent,
                        begin_rowid,
                        end_rowid,
                    )
                else:
                    row_query = """select philo_name,lemma from words where parent='%s'""" % (parent,)
                cursor.execute(row_query)
                for i in cursor:
                    main_query = request["q"].split(" ")[0]
                    collocate = i["philo_name"]

                    lemma = i["lemma"] or ""
                    if request["lemmas"] == "yes" or "lemma" in main_query: use_lemmas = True
                    else: use_lemmas = False

                    if collocate not in filter_list and "lemma:" + lemma not in filter_list and collocate != main_query and lemma != main_query.replace("lemma:","") and "pos" not in collocate:
                        if not use_lemmas and "lemma" not in collocate:
                            stored_sentence_counts[collocate] += 1
                        elif use_lemmas:
                            stored_sentence_counts[lemma] += 1
