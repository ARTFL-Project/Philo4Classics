...
def translit_normalized_word_frequencies(loader_obj):
    print("%s: Generating normalized word frequencies..." % time.ctime())
    frequencies = loader_obj.destination + "/frequencies"
    output = open(frequencies + "/normalized_word_frequencies", "w")
    for line in open(frequencies + "/word_frequencies"):
        ...
        #Transliterate Greek to Latin (WMS)
        greek_alphabet = 'ΑαΒβΓγΔδΕεΖζΗηΘθΙιΚκΛλΜμΝνΞξΟοΠπΡρΣσςΤτΥυΦφΧχΨψΩω'
        latin_alphabet = 'AaBbGgDdEeZzHhQqIiKkLlMmNnCcOoPpRrSssTtUuFfXxYyWw'
        greek2latin = str.maketrans(greek_alphabet, latin_alphabet)
        translit = norm_word.translate(greek2latin)

        #print(norm_word + "\t" + word, file=output)
        print(norm_word + "\t" + word + "\t" + translit, file=output)
    output.close()
...
PerseusPostFilters = [
    word_frequencies,
#    translit_normalized_word_frequencies,
#    translit_normalized_metadata_frequencies,
    normalized_word_frequencies,
    metadata_frequencies,
    normalized_metadata_frequencies,
]

def set_default_postfilters(postfilters=PerseusPostFilters):
    filters = []
