...
milestone_chapter_tag = re.compile(r'<milestone.*=\"(chapter|page|cap)', re.I)
milestone_section_tag = re.compile(r'<milestone.*\"(section|par)', re.I)
...
book_attribute = re.compile(r'<.*?type=\"([Bb]ook|lib)\".*?n=\"(.*?)\"', re.I)
...
speaker_tag = re.compile(r"<(q|sp)\W", re.I)
closed_speaker_tag = re.compile(r"</(q|sp)>", re.I)
...
div_section_tag = re.compile(r'<div.*\"section', re.I)
...
class XMLParser:
    ...
    def parse(self, text_input):
        ...
        try:
            ...
            self.v["doc"]["abbrev"] = self.urn[0]
            # if what the abbrevs file has is editions, then convert to regular urn
            if "-" in self.urn[1]:
                self.v["doc"]["cts_urn"] = re.sub(r'(urn:cts:\w+:\w+\.\w+).[\w-]+:*([0-9\.]+)*', r'\1', self.urn[1])
            else:
                self.v["doc"]["cts_urn"] = self.urn[1]
        except KeyError as e:
            print("No abbreviation found for %s" % str(e))
        ...
        for line in self.content:
            ...
            if self.in_the_text is False:
                if text_tag.search(line) or doc_body_tag.search(line) or body_tag.search(line):
                    ...
                    for i in range(1,4):
                        self.v["doc"]["cts_div" + str(i)] = ''.join([k for k,v in self.refStates.items() if v["level"] == i])
            ...
            if line.startswith("<"):
                ...
                if self.in_the_text:
                    ...
                    # When in header, let's checks for refsDecl (WMS)
    ...
    def tag_handler(self, tag):
        ...
        if tag_name.replace("/", "") == self.current_suppressed_tag and tag.startswith("</"):
            ...
            # I commented out the following since we probably don't want <add> tags
            # to act as paragraph enders (WMS)
            ...
            elif div_section_tag.search(tag):
                if (self.got_a_div2 and self.got_a_milestone) or self.got_a_div:
...
#            elif milestone_bekker_tag.search(tag) or milestone_poem_tag.search(tag) or milestone_card_tag.search(tag) or milestone_chapter_tag.search(tag):
            ...
            #   the Philoid and head for searching under document levels.
            ...
            elif (div_tag.search(tag) and (not self.is_drama or (self.is_drama and not self.using_cards)) and not div_edition_tag.search(tag) and not div_translation_tag.search(tag) and not div_card_tag.search(tag) and not div_dsection_tag.search(tag)) and not div_chapter_tag.search(tag) and not div_chapter_tag.search(tag) and not div_section_tag.search(tag):
                ...
                # if numeric divs are being used, then override the refState levels
...
#                elif div_type == "div2" and not self.using_cards:
                    ...
                    #print("div2 attribs %s" % self.v["div2"])
                else:
                    if self.open_div3:
                        self.close_div3(start_byte)
                    self.open_div3 = True
                    ...
                    #print("div3 attribs %s" % self.v["div3"])
                ...
                # The next line is a workaround for AristophanesAcharniansEn for crazy <date> tags
                self.context_div_level = 1
            ...
            if read_more:
                while read_more:
                    ...
                    if closed_head_tag.search(next_line):
                        ...
                        if not div_head or any_tag.search(div_head):
                            div_head = next_line
                        else:
                            break
        if div_head:
